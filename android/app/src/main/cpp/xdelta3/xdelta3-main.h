...
void xprintf(const char *fmt, ...) {
  ...
  __android_log_print(ANDROID_LOG_ERROR, "xdelta3", "%s", buf);
...
int main_file_open(main_file *xfile, const char *name, int mode) {
  ...
  if ((ret = open(name, XOPEN_POSIX, XOPEN_MODE)) < 0) {
    ret = get_errno();
    __android_log_print(ANDROID_LOG_ERROR, "xdelta3", "open failed: path=%s mode=%d errno=%d (%s)", name, mode, ret, strerror(ret));
  } else {
    xfile->file = ret;
    ret = 0;
  }
